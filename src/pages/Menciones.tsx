...
import { AtSign, ChevronRight, Eye, EyeOff, Loader2 } from "lucide-react";
...
export default function Menciones() {
  ...
  const { data: mentions = [], isLoading } = useMyMentions(user?.id);
  const toggleRead = useToggleMentionRead();
  const [tab, setTab] = useState<Tab>("no_leidas");

  const filtered = mentions.filter((m) =>
    tab === "todas" ? true : tab === "leidas" ? m.is_read : !m.is_read,
  );
              ...
              <ul className="divide-y divide-border">
                {g.rows.map((m) => (
                  <li key={m.id} className={cn("flex items-start gap-3 px-4 py-2", !m.is_read && "bg-primary/5")}>
                    <button
                      onClick={() => user && toggleRead.mutate({ mentionId: m.id, userId: user.id, read: !m.is_read })}
                      className={cn("mt-0.5 p-1 rounded hover:bg-muted", m.is_read ? "text-muted-foreground" : "text-primary")}
                      title={m.is_read ? "Marcar como no leída" : "Marcar como leída"}
                    >
