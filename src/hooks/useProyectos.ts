...
export type ProyectoWithEmpresas = ProyectoRow & {
  ...
  proyecto_clientes?: { id: string; cliente_id: string; contacto_id: string | null; clientes: { id: string; nombre: string; categoria_id: string } | null; contacto: { id: string; contacto: string } | null }[];
    ...
    queryFn: async () => {
      ...
      while (true) {
        const { data: page, error } = await supabase
          .from("proyectos")
          .select("*, proyecto_empresas(*, empresas(*), categorias_proyecto(*), subcategorias_proyecto(*)), clasificaciones_proyecto(*), proyecto_clientes(id, cliente_id, contacto_id, clientes(id, nombre, categoria_id), contacto:contactos_cliente(id, contacto)), proyecto_captadores(id, captador_id, captadores(id, nombre))")
